...
当前旧 Agent 入口通过 `v1alpha1.WithExecutionContext` 传递可信上下文；兼容层暂时把已有 `shop_id` 同时映射为 `merchant_id` 和 `location_id`。独立商户 ID、顾客 ID 和写操作幂等键必须在应用服务完成真实解析后再填充，不能由模型参数补入。
