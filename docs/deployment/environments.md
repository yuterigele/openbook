...
OpenBook 运行环境固定为 `development`、`staging`、`production`。三者使用同一份镜像和 Docker Compose 定义，差异只通过环境变量与独立基础设施体现；禁止把生产凭据复制到开发或预发布环境。
...
1. `docker compose --env-file <目标文件> config -q`。
